...
function anggotaMahasiswaPerRuang($idRuang) {
    ...
    $db_pass = "375300";
    $con = konekDb($db_user, $db_pass);
    $sql = "select X.nama, A.anggota, A.statusanggota, A.keanggotaan from anggotaLab A INNER JOIN mahasiswa X on A.anggota = X.nrp WHERE RUANG = $idRuang";
    $hasil = query_getAll($con, $sql);
    return $hasil;
}
...
function anggotaPegawaiPerRuang($idRuang) {
    ...
    $db_pass = "375300";
    $con = konekDb($db_user, $db_pass);
    $sql = "select X.nama, A.anggota, A.statusanggota, A.keanggotaan from anggotaLab A INNER JOIN pegawai X on A.anggota = X.nip WHERE RUANG = '$idRuang'";
    $hasil = query_getAll($con, $sql);
