...
function anggotaMahasiswaPerRuang($idRuang) {
    ...
    $db_pass = "375300";
    $con = konekDb($db_user, $db_pass);
    $sql = "select A.nomor, X.nama, A.anggota, A.statusanggota, A.keanggotaan from anggotaLab A INNER JOIN mahasiswa X on A.anggota = X.nrp WHERE RUANG = $idRuang AND A.KEANGGOTAAN = 1";
    $hasil = query_getAll($con, $sql);
    return $hasil;
}
...
function anggotaPegawaiPerRuang($idRuang) {
    ...
    $db_pass = "375300";
    $con = konekDb($db_user, $db_pass);
    $sql = "select A.nomor, X.nama, A.anggota, A.statusanggota, A.keanggotaan from anggotaLab A INNER JOIN pegawai X on A.anggota = X.nip WHERE RUANG = '$idRuang' AND A.KEANGGOTAAN = 1";
    $hasil = query_getAll($con, $sql);
